...
app.get("/api/admin/sample-data/data", async (_req, res) => {
  try {
    const result = await pool.query(
      `SELECT code AS id, deal_name, sector, geography, status
       FROM deals
...
app.get("/api/deals/stats", async (_req, res) => {
  try {
    const result = await pool.query<{
      deals_screened: string;
      high_risk_flags: string;
      ic_packs_ready: string;
    }>(`
      SELECT
...
function normalizeDealCode(rawDealId: string) {
  return rawDealId.replace(/-/g, "_").toUpperCase().replace(/_/g, "-");
}

function coordinateFromRef(ref: string): EvidenceHighlight {
  let hash = 0;
...
function normalizeCoordinates(input: unknown): EvidenceHighlight[] {
  ...
  return references
    .map((entry) => {
      const candidate = entry as Partial<EvidenceHighlight>;
      if (
    ...
    .filter((value): value is EvidenceHighlight => Boolean(value));
}
...
async function processIngestionJob(input: StartIngestionInput) {
  ...
  try {
    ...
    await pool.query(
      ...
      [input.dealCode, input.dealCode]
    );

    const dealResult = await pool.query<{ id: string }>(
      `SELECT id FROM deals WHERE code = $1 LIMIT 1`,
      [input.dealCode]
    );
    ...
    const metricsByYear = new Map<number, Record<string, any>>();
...
app.post("/api/deals/ingest", async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO ingestion_jobs (job_id, deal_code, status)
       VALUES ($1, $2, 'QUEUED')`,
      [jobId, dealCode]
    );
...
app.get("/api/deals/ingest/:jobId/status", async (req, res) => {
  try {
    const result = await pool.query<{
      ...
      deal_code: string;
      status: IngestJobStatus;
      datagol_extraction_id: string | null;
      metrics_saved: number;
      risk_flags_saved: number;
      error: string | null;
      created_at: string;
      updated_at: string;
    }>(
      `SELECT job_id, deal_code, status, datagol_extraction_id, metrics_saved, risk_flags_saved, error, created_at, updated_at
       FROM ingestion_jobs
       WHERE job_id = $1
       LIMIT 1`,
      [req.params.jobId]
...
app.get("/api/deals/:dealId/evidence", async (req, res) => {
  try {
    ...
    const dealResult = await pool.query<{
      ...
      `SELECT id, code, deal_name, sector, status
       FROM deals
       WHERE code = $1
       LIMIT 1`,
      [normalizedCode]
    ...
    const metricsResult = await pool.query<{
      ...
      `SELECT fiscal_year, revenue, gross_profit, adjusted_ebitda, capex, evidence_ref
       FROM deal_metrics
       WHERE deal_id = $1
    ...
    const riskRefsResult = await pool.query<{ source_reference_id: string | null; evidence_ref: unknown }>(
      `SELECT source_reference_id, evidence_ref
       FROM risk_flags
       WHERE deal_id = $1
       ORDER BY created_at DESC`,
      [deal.id]
    ...
    const highlights = Object.fromEntries(
      kpis.flatMap((kpi) =>
        kpi.evidenceOptions.map((option) => [
          option.id,
          {
            page: option.page,
            x: option.x,
    ...
    ) as Record<string, EvidenceHighlight>;

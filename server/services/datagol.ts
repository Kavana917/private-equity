...
export type DatagolUploadResult = {
  fileId: string;
  raw: unknown;
};

export type DatagolTriggerResult = {
  requestId: string;
  raw: unknown;
};

export type DatagolPollResult = {
  requestId: string;
  status: string;
  completed: boolean;
  failed: boolean;
...
const defaultOptions: DatagolClientOptions = {
  ...
  uploadEndpoint:
    process.env.DATAGOL_UPLOAD_ENDPOINT ?? "/noCo/api/v2/workspaces/{workspaceId}/file/upload"
};

function requiredOption(value: string, key: string) {
  if (!value) {
...
function toWorkspaceFile(entry: Record<string, unknown>): DatagolWorkspaceFile | null {
...
function extractFolderIds(entries: Record<string, unknown>[]): string[] {
  const folderIds = new Set<string>();
  for (const entry of entries) {
    const entryTypeCandidate = entry.elementType ?? entry.type ?? entry.resourceType;
    const looksLikeFolder =
      typeof entryTypeCandidate === "string" && entryTypeCandidate.toUpperCase().includes("FOLDER");
    if (!looksLikeFolder) {
      continue;
    }
...
export class DatagolService {
  private readonly options: DatagolClientOptions;

  constructor(options?: Partial<DatagolClientOptions>) {
  ...
  async uploadDocument(fileBuffer: Buffer, fileName: string, mimeType: string): Promise<DatagolUploadResult> {
    ...
    const payload = await requestDatagol(
      uploadPath,
      {
        method: "POST",
        body: form
  ...
  async listWorkspaceFiles(): Promise<DatagolWorkspaceFile[]> {
    ...
    const parsedResults: DatagolWorkspaceFile[] = [];
    ...
    for (const path of candidatePaths) {
      try {
        ...
        parsedResults.push(...rows.map(toWorkspaceFile).filter((value): value is DatagolWorkspaceFile => Boolean(value)));
        if (parsedResults.length > 0) {
          break;
        }
      } catch (error) {
        endpointErrors.push(
          `${path}: ${error instanceof Error ? error.message : "unknown Datagol list error"}`
        );
      }
    }
    ...
    if (parsedResults.length === 0) {
      try {
        const folderPayload = await requestDatagol(
          `/noCo/api/v2/workspaces/${this.options.workspaceId}/folder`,
          { method: "GET" },
          this.options.apiToken,
          this.options.baseUrl,
        ...
        for (const folderId of folderIds) {
          ...
          for (const folderFilePath of folderFilePaths) {
            try {
              const folderFilesPayload = await requestDatagol(
                folderFilePath,
                { method: "GET" },
                this.options.apiToken,
                this.options.baseUrl,
              ...
              parsedResults.push(
                ...folderFileRows
                  .map(toWorkspaceFile)
                  .filter((value): value is DatagolWorkspaceFile => Boolean(value))
              );
              if (parsedResults.length > 0) {
                break;
              }
        ...
        endpointErrors.push(
          `/folder fallback: ${error instanceof Error ? error.message : "unknown Datagol folder list error"}`
        );
      }
    }

    const deduped = new Map<string, DatagolWorkspaceFile>();
  ...
  async triggerExtraction(extractionId: string, elementType: "FILE" | "FOLDER" | "COLUMN", elementId: string) {
    ...
    return { requestId, raw: payload } satisfies DatagolTriggerResult;
  }

  async pollExtractionStatus(requestId: string): Promise<DatagolPollResult> {
    const startedAt = Date.now();
    while (Date.now() - startedAt < this.options.pollTimeoutMs) {
      const payload = await requestDatagol(
        `/noCo/api/v1/requestLog/status/${requestId}`,
        { method: "GET" },
        this.options.apiToken,
        this.options.baseUrl,
        this.options.requestTimeoutMs
  ...
  async getExtractionResults(requestId: string) {
    const payload = await requestDatagol(
      `/noCo/api/v1/requestLog/status/${requestId}`,
      { method: "GET" },
      this.options.apiToken,
